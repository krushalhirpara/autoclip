    ...
    <main className="min-h-screen bg-[#F8F9FC] pb-24 pt-24 selection:bg-[#7C5CFC]/20 dark:bg-[#0A0A0C] overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 sm:px-8">
        ...
        <div className="mb-12 mt-8 flex items-center space-x-2 text-xs font-semibold tracking-wide text-[#6B6B78] dark:text-[#A1A1AA]">
          ...
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="text-[#111118] dark:text-white">{String("For YouTubers")}</span>
        </div>
        ...
        <div className="flex flex-col gap-16 lg:flex-row lg:items-center lg:gap-20">
          <div className="flex-1 text-center lg:text-left">
            ...
            <h1 className="mt-8 text-4xl font-black tracking-tight text-[#111118] dark:text-white sm:text-6xl lg:text-[4rem] lg:leading-[1.1]">
              {String("For YouTubers")}
            </h1>
            <p className="mt-6 max-w-2xl text-lg font-medium leading-relaxed text-[#6B6B78] dark:text-[#A1A1AA] mx-auto lg:mx-0">
              {String("Repurpose your long-form YouTube videos to grow your Shorts channel and TikTok following.")}
            </p>
            ...
            <div className="mt-10 flex flex-wrap gap-4 justify-center lg:justify-start">
              <Button size="lg" className="rounded-full bg-gradient-to-r from-[#7C5CFC] to-[#9B7CFF] px-8 text-base font-bold text-white shadow-[0_4px_20px_rgba(124,92,252,0.4)] hover:shadow-[0_8px_30px_rgba(124,92,252,0.6)] transition-all duration-300 hover:-translate-y-1" asChild>
                <Link href="{String('/signup')}">
                  {String("Boost Your Views")} <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
            </div>
      ...
      <div className="w-full flex flex-col space-y-4 items-center">
        <div className="w-full">
          <VideoThumbnail orientation="landscape" duration="15:30" label="YOUTUBE LONG FORM" className="w-full shadow-md" />
        </div>
        <ArrowRight className="w-5 h-5 text-[#7C5CFC]" />
        <div className="flex gap-2">
           <VideoThumbnail orientation="portrait" duration="00:45" label="" className="w-16 h-24" />
           <VideoThumbnail orientation="portrait" duration="00:30" label="" className="w-16 h-24" />
           <VideoThumbnail orientation="portrait" duration="00:60" label="" className="w-16 h-24" />
        </div>
      </div>
        ...
        <div className="mt-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 relative">
            ...
            {["Paste URL","AI Analyzes","Best Hooks Found","Captions Added","Publish"].map((step, idx, arr) => (
        ...
        <div className="mt-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[{"title":"Direct URL Import","desc":"Just paste your YouTube link. No need to download and re-upload gigabytes of video files."},{"title":"High-Retention Hooks","desc":"Our AI specifically searches for high-energy moments that perform well in the Shorts algorithm."},{"title":"End Screen Safe","desc":"Automatic padding ensures your captions don't overlap with YouTube Shorts UI elements."}].map((benefit, idx) => (
