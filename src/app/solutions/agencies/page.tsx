    ...
    <main className="min-h-screen bg-[#F8F9FC] pb-24 pt-24 selection:bg-[#7C5CFC]/20 dark:bg-[#0A0A0C] overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 sm:px-8">
        ...
        <div className="mb-12 mt-8 flex items-center space-x-2 text-xs font-semibold tracking-wide text-[#6B6B78] dark:text-[#A1A1AA]">
          ...
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="text-[#111118] dark:text-white">{String("For Agencies")}</span>
        </div>
        ...
        <div className="flex flex-col gap-16 lg:flex-row lg:items-center lg:gap-20">
          <div className="flex-1 text-center lg:text-left">
            ...
            <p className="mt-6 max-w-2xl text-lg font-medium leading-relaxed text-[#6B6B78] dark:text-[#A1A1AA] mx-auto lg:mx-0">
              {String("Scale multi-client video clipping pipelines with multi-tenant workspaces and custom brand templates.")}
            </p>
            ...
            <div className="mt-10 flex flex-wrap gap-4 justify-center lg:justify-start">
              <Button size="lg" className="rounded-full bg-gradient-to-r from-[#7C5CFC] to-[#9B7CFF] px-8 text-base font-bold text-white shadow-[0_4px_20px_rgba(124,92,252,0.4)] hover:shadow-[0_8px_30px_rgba(124,92,252,0.6)] transition-all duration-300 hover:-translate-y-1" asChild>
                <Link href="{String('/signup')}">
                  {String("Scale Your Agency")} <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
            </div>
      ...
      <div className="w-full h-full flex flex-col space-y-2">
        ...
        <div className="flex-1 bg-white dark:bg-[#111118] rounded-xl border border-[#E8E7F0] dark:border-[#27272A] p-4 flex flex-col space-y-3">
          ...
          <div className="flex gap-2">
            <VideoThumbnail orientation="portrait" duration="00:15" label="" className="w-10 h-14 rounded" />
            <VideoThumbnail orientation="portrait" duration="00:20" label="" className="w-10 h-14 rounded" />
            <VideoThumbnail orientation="portrait" duration="00:25" label="" className="w-10 h-14 rounded" />
            <VideoThumbnail orientation="portrait" duration="00:30" label="" className="w-10 h-14 rounded" />
          </div>
        </div>
      </div>
        ...
        <div className="mt-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 relative">
            ...
            {["Client Workspaces","Brand Kits","Bulk Processing","Client Approval","Delivery"].map((step, idx, arr) => (
        ...
        <div className="mt-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[{"title":"Multi-Tenant Workspaces","desc":"Keep client projects, billing, and assets completely separated in dedicated workspaces."},{"title":"Approval Workflows","desc":"Share a white-labeled link with clients where they can leave comments and approve clips."},{"title":"Unlimited Brand Kits","desc":"Store unlimited fonts, color palettes, and logo watermarks for all your different clients."}].map((benefit, idx) => (
